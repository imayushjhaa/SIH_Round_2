...
function MapController({ selectedPlot, geoData }) {
  ...
  useEffect(() => {
    ...
    try {
      ...
      console.error("Auto bounds error:", err);
    }
  }, [geoData, map]);
...
function StatutoryTimelineModule({ plotInfo }) {
  ...
  return (
    <div className="p-3 rounded-lg border space-y-2.5 bg-slate-50 border-slate-200">
      <div className="flex justify-between items-center">
        <span className="text-[11px] font-bold tracking-wide uppercase flex items-center gap-1.5 text-slate-700">
            ...
            <div key={stg.id} className="relative z-10 flex flex-col items-center">
              ...
              <span 
                className={`text-[9px] mt-1 font-semibold whitespace-nowrap ${
                  isCurrent 
                    ? isLapsed ? "text-red-600 font-bold" : "text-blue-600 font-bold" 
                    : isDone 
                    ? "text-emerald-600" 
                    : "text-slate-500"
                }`}
              ...
              <span className="text-[8px] scale-90 -mt-0.5 text-slate-500">{stg.window}</span>
            </div>
          );
        })}
      </div>
      ...
      <div className={`p-2 rounded border text-xs space-y-1 ${
        ...
        <div className="flex items-center justify-between font-medium text-[11px]">
          <span className="flex items-center gap-1">
            <Calendar size={11} className="text-slate-500" /> Sec 11 Notification:
          </span>
        ...
        <div className="flex items-center justify-between font-medium text-[11px]">
          <span className="flex items-center gap-1">
            <Clock size={11} className="text-slate-500" /> Sec 19 Deadline:
          </span>
          <span className="font-semibold text-slate-900">{deadline}</span>
        </div>

        <div className="pt-1 border-t border-slate-200/60 flex items-center justify-between font-bold text-[11px]">
...
function ResetViewButton({ geoData }) {
  ...
  const resetView = () => {
    ...
    try {
      ...
      console.error("Reset bounds error:", err);
    }
  };
  ...
  return (
    <div className="absolute top-3 right-3 z-[1000]">
      <button
        onClick={resetView}
        className="bg-slate-900/90 hover:bg-slate-800 text-white text-xs font-semibold px-3 py-2 rounded-xl border border-slate-700 shadow-xl backdrop-blur-md flex items-center gap-1.5 transition-all cursor-pointer"
        title="Reset map view to corridor cluster"
      >
        <Compass size={14} className="text-amber-400" /> Reset View
      </button>
    </div>
  );
}
...
export default function App() {
  ...
  const handleMouseMove = (e) => {
    ...
    if (newMapWidthPercent > maxAllowedPercent) {
      newMapWidthPercent = maxAllowedPercent;
    }

    setMapWidth(newMapWidthPercent);
  };
  ...
  const polygonStyle = (feature) => {
    ...
    return {
      ...
      color: isSelected ? "#0284c7" : isStatutoryCritical ? "#ef4444" : isSimulatedPlot ? "#22c55e" : "#ffffff",
    ...
    <div className="flex flex-col h-screen font-sans bg-slate-100 text-slate-800 selection:bg-amber-200">
      ...
      <div className="grid grid-cols-5 gap-2.5 px-5 py-2 border-b bg-white border-slate-200">
        ...
        <div 
          ...
          className={`flex items-center justify-between px-3 py-1.5 rounded-lg cursor-pointer select-none transition-all ${
            activeFilter === "ALL" 
              ? "bg-slate-100 border-2 border-slate-700 shadow-sm ring-2 ring-slate-400/20 scale-[1.01]" 
              : "bg-slate-50 border border-slate-200 hover:bg-slate-100/70 hover:border-slate-300"
          }`}
        >
          <div>
            <div className="flex items-center gap-1">
              <p className="text-[10px] font-bold uppercase tracking-wider text-slate-700">Total Parcels</p>
              {activeFilter === "ALL" && (
                <span className="text-[8px] bg-slate-200 text-slate-800 px-1 py-0.2 rounded font-bold uppercase">Active</span>
              )}
            </div>
            <p className="text-[10px] text-slate-500">All corridor plots</p>
          </div>
          ...
          <div>
            <div className="flex items-center gap-1">
              <p className="text-[10px] font-bold uppercase tracking-wider text-red-600 flex items-center gap-1">
                <AlertTriangle size={11} className="text-red-600" /> Sec 19 Lapse
              </p>
            ...
            <p className="text-[10px] text-red-500">Click to filter critical</p>
          </div>
        ...
        >
          <div>
            <div className="flex items-center gap-1">
              <p className="text-[10px] font-bold uppercase tracking-wider text-amber-700">High Delay Risk</p>
              {activeFilter === "HIGH_RISK" && (
                <span className="text-[8px] bg-amber-600 text-white px-1 py-0.2 rounded font-bold uppercase">Active</span>
              )}
            </div>
            <p className="text-[10px] text-amber-600">&gt;90d delay plots</p>
          </div>
          ...
          <div>
            <div className="flex items-center gap-1">
              <p className="text-[10px] font-bold uppercase tracking-wider text-emerald-700 flex items-center gap-1">
                <CheckCircle2 size={11} className="text-emerald-600" /> Safe / Cleared
              </p>
              {activeFilter === "SAFE" && (
                <span className="text-[8px] bg-emerald-600 text-white px-1 py-0.2 rounded font-bold uppercase">Active</span>
              )}
            </div>
            <p className="text-[10px] text-emerald-600">On track, dispute-free</p>
          </div>
        ...
        >
          <div>
            <div className="flex items-center gap-1">
              <p className="text-[10px] font-bold uppercase tracking-wider text-slate-700">Disbursement</p>
              <span className="text-[8px] font-semibold px-1 py-0.2 rounded bg-slate-200 text-slate-700">Target: 80%</span>
            </div>
            <p className="text-[10px] text-slate-500">Award cleared</p>
          </div>
      ...
      <div ref={containerRef} className="flex flex-1 overflow-hidden p-3 gap-0 relative">
        ...
        >
          ...
          {geoData && (
            ...
            >
              ...
              <ResetViewButton geoData={geoData} />
              <TileLayer
        ...
        <div
          ...
          title="Drag left or right to resize map & details panel"
        >
        ...
        >
          {!selectedPlot ? (
            <div className="h-full flex flex-col items-center justify-center text-center">
              <Layers size={44} className="mb-2 text-slate-300" />
              <p className="font-medium text-sm text-slate-600">Select any Land Parcel (Khasra) on the map</p>
            ...
            <>
              ...
              <div className="border-b pb-2.5 border-slate-200">
                <div className="flex justify-between items-start">
                  <div>
                    <h2 className="text-base font-bold text-slate-900">{plotDetails.plot_info.khasra_no}</h2>
                    <p className="text-xs text-slate-500">Village: {plotDetails.plot_info.village} | {plotDetails.plot_info.project}</p>
                  </div>
                  <span className={`text-[11px] px-2 py-0.5 rounded font-semibold ${
                    simResult ? "bg-emerald-100 text-emerald-700 border border-emerald-300" :
                    ...
                    plotDetails.plot_info.risk_tier === "High" ? "bg-amber-100 text-amber-700 border border-amber-300" : "bg-emerald-100 text-emerald-700 border border-emerald-300"
                  }`}>
              ...
              {isPlotLapsed ? (
                <div className="bg-red-900 text-white p-3 rounded-lg border border-red-700 space-y-1 shadow-inner">
                  <div className="flex items-center justify-between">
                    <span className="flex items-center gap-1.5 text-red-200 text-xs font-bold uppercase tracking-wider">
                      <AlertOctagon size={14} /> Statutory Proceeding Abated
                    </span>
                    <span className="text-[10px] bg-red-950 text-red-200 px-2 py-0.5 rounded font-mono font-bold">
                      Sec 19(7) VOID
                    </span>
                  </div>
                ...
                <div className="bg-slate-900 border border-slate-800 text-white p-2.5 rounded-lg flex items-center justify-between">
                  <div>
              ...
              <div>
                <div className="flex justify-between items-center mb-1.5">
                  ...
                  {isPlotLapsed && (
                    <span className="text-[9px] text-red-600 font-semibold bg-red-50 px-1.5 py-0.2 rounded border border-red-200">
                ...
                <div className="space-y-1.5">
                  {plotDetails.shap_breakdown.map((item, idx) => (
                    <div key={idx} className="text-xs p-2 rounded border bg-slate-50 border-slate-200 text-slate-700">
                      <div className="flex justify-between font-medium mb-1">
                        <span className="capitalize text-[11px]">{item.factor.replace(/_/g, " ")}</span>
                        <span className="text-red-600 font-bold text-[11px]">+{item.impact_days}d ({item.contribution_pct}%)</span>
                      </div>
              ...
              <div className="p-3 rounded-lg space-y-1.5 border bg-amber-50 border-amber-300 text-amber-900">
                <div className="flex items-center justify-between">
                  <span className="flex items-center gap-1.5 font-bold text-[11px] uppercase">
                    <CheckCircle2 size={14} className="text-amber-600" /> Prescriptive Administrative Action
                  </span>
                  <span className="flex items-center gap-1 text-[9px] font-bold px-1.5 py-0.2 rounded-full border bg-amber-200/80 border-amber-300 text-amber-900">
                    <Sparkles size={10} className="text-amber-600" /> GenAI Ground Plan
                  </span>
                </div>
                ...
                {plotDetails.ai_mitigation_steps && plotDetails.ai_mitigation_steps.length > 0 ? (
                  <div className="space-y-1.5 pt-0.5">
                    {plotDetails.ai_mitigation_steps.map((step, idx) => (
                      <div key={idx} className="text-[11px] flex items-start gap-1.5 p-2 rounded border shadow-xs bg-white border-amber-200 text-slate-700">
                        <span className="font-bold text-amber-600 min-w-[14px]">{idx + 1}.</span>
                ...
                <div className="text-[10px] font-medium pt-0.5 opacity-90 text-slate-700">
              ...
              <div className="p-3 rounded-lg space-y-2.5 border bg-slate-50 border-slate-300">
                <div className="flex items-center justify-between">
                  <div className="flex items-center gap-1.5 font-bold text-xs text-slate-800">
                    <Sliders size={14} className="text-slate-600" /> "What-If" Mitigation Simulator
                  </div>
                  {simResult && (
                    <button
                ...
                {isPlotLapsed ? (
                  <div className="p-2 rounded text-[11px] border bg-red-100/70 border-red-200 text-red-800">
                  ...
                  <>
                    <div>
                      <div className="flex justify-between text-[11px] mb-1 text-slate-700 font-medium">
                      ...
                      <input
                        ...
                        className="w-full cursor-pointer h-2 rounded-lg appearance-none bg-slate-300 accent-blue-600 focus:outline-none"
                        style={{
                          background: `linear-gradient(to right, #2563eb ${simDisbursement}%, #cbd5e1 ${simDisbursement}%)`
                        }}
                      />
                    </div>
                    ...
                    <button
                      ...
                      className="w-full bg-blue-600 hover:bg-blue-700 text-white text-xs font-semibold py-1.5 rounded transition-colors cursor-pointer shadow-sm"
                    >
                      Simulate Intervention
                    </button>

                    {simResult && (
                      <div className="mt-1.5 p-2 rounded text-xs space-y-0.5 border bg-emerald-50 border-emerald-300 text-emerald-800">
